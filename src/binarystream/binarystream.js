...
class BinaryStream {
  ...
  readUnsignedVarInt() {
    ...
    for (let i = 0; i <= 28; i += 7) {
      if (this.feof) throw new Error("No bytes left in buffer.");
      
      let rByte = this.readByte();
      value |= ((rByte & 0x7f) << i);
      
      if ((rByte & 0x80) === 0) return value;
  ...
  writeUnsignedVarInt(value) {
    ...
    for (let i = 0; i < 5; i++) {
      if ((value >> 7) !== 0) {
        ...
        this.writeData(binstream);
        return;
      }
      
      value = ((value >> 7) & (0x7fffffff >> 6));
    }

...
class BinaryStream {
  ...
  readData(length, offsetChange = true) {
    if(length === 0) return "Error: Provide a specific length.";
  ...
  writeString(value) {    
    ...
    binstream.writeShort(Buffer.byteLength(value));
    binstream._append(Buffer.from(value, "utf-8"));
    
    return this.writeData(binstream.buffer);
  }
}
